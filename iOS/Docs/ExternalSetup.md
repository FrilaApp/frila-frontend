...
11. Concluído em 24/09: o `frila-dev` está em dia com o `main` do backend. Tem 29 migrações e 18 RPCs (conta, perfil, estabelecimento e painel, vaga, candidatura, turno, contato, check-in, check-out e cancelamentos). Sem sessão elas respondem `401` com `42501`, que o app trata como não autenticado. Ainda não existem no backend `configuracao_do_app`, que responde `404 PGRST202`, e `avaliar`. O despacho (`despachar`) é do Sprint 2. O espelho do contrato no iOS e o do backend estão na 0.2.11.
