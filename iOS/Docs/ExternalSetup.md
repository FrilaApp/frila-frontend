...
10. Concluído no iOS: cliente sincronizado com o contrato `0.2.11` publicado no frila-docs (commit `2c7151f`), com `meusTurnos()` chamando `meus_turnos`. A atualização não mudou schemas nem payloads usados pelo app; os novos códigos de erro estão tipados em `CodigoErroAPI`.
